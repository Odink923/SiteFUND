...
const ReviewsCarrousel = () => {
    ...
    return (
                ...
                {slides.map((slide, index) => (
                    <SwiperSlide key={index}>
                        <div className="relative pl-15 pr-15 ml-[2.875rem] mr-[2.875rem] bg-[#20202026] lg:w-[63.23vw] lg:h-[20.16vw] lg:ml-[4.21vw] rounded-[3.13vw] ">
                            {slide.title && (
                                <div className="text-[#202020] pt-20 lg:pt-[4.58vw]  lg:pl-[4.27vw] lg:w-[45.78vw] lg:h-[10.00vw] leading-none ">

                                    <div className="flex items-center pb-15 lg:pb-[0px]">
                                        ...
                                        <h2 className="text-base sm:text-smBase pl-10  lg:text-[1.88vw] lg:pl-[1.82vw] font-assistant font-semibold">{slide.title}</h2>
                                    </div>

                                    <p className="text-base sm:text-smBase pb-20 lg:pb-[0px] lg:text-[1.04vw] lg:pt-[1.82vw] text-justify">{slide.description}</p>
                                </div>
                            )}
                        </div>
                    </SwiperSlide>
        ...
        "
                ...
                {/* Buttons for screens smaller than LG */}
                <button 
    onClick={() => swiperRef.current && swiperRef.current.slidePrev()}
    className="
        lg:hidden absolute left-1 top-1/3 transform -translate-y-1/3 text-[#2E2E2E] rounded-full focus:outline-none transition-colors duration-300
        w-[30px] h-[30px] flex items-center justify-center z-10
    "
>
    <FaChevronLeft className="text-[40px] font-thin" />
</button>
    ...
    <button
        onClick={() => swiperRef.current && swiperRef.current.slideNext()}
        className="
            lg:hidden absolute right-2 top-1/3 transform -translate-y-1/3  text-[#2E2E2E] rounded-full focus:outline-none transition-colors duration-300  
            w-[30px] h-[30px] flex items-center justify-center z-10
        "
    >
        <FaChevronRight className="text-[40px] font-thin " />
    </button>   
            </div>
        </div>
    );
